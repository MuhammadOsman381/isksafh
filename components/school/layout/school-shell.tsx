...
export function SchoolShell({
  ...
  const visibleTabs = tabs.filter((tab) => {
    ...
    if (role === "teacher") return ["dashboard", "students", "marks"].includes(tab.id);
  ...
  return (
    <main className="min-h-screen bg-[#f7f7f4] text-zinc-950">
      <div className="grid min-h-screen lg:grid-cols-[280px_1fr]">
        <aside className="border-r border-zinc-200 bg-zinc-950 px-5 py-5 text-white lg:sticky lg:top-0 lg:h-screen">
          <div className="flex items-center gap-3">
            <div className="flex size-11 items-center justify-center rounded-2xl bg-emerald-400 text-zinc-950">
        ...
        <section className="min-w-0 px-4 py-4 sm:px-6 lg:px-8">
          <header className="flex flex-col gap-4 border-b border-zinc-200 pb-5 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <StatusPill icon={Database} label={source === "neon" ? "Neon connected" : "Demo mode"} />
